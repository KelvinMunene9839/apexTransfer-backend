...
// Same missing-authorization gap as settle_deferred_transaction (its
// RWF-quoted sibling) — see that route for the full explanation.
router.post('/', requireUser, validateBody(validateParams), asyncWrapper(async (req, res) => {
  ...
  const { error } = await callRpcAsService('settle_deferred_transaction_fx', {
